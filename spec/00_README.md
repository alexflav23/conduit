...
| 24 | **CONTRACT_PRICING** | Contract & volume-tiered pricing (ADLP re-stated): nobody types a price — every price is a governed tier in a `price_agreement` (validity, multi-customer scope, volume bands); the "exception" is a governed price-tier request; per-order vs cumulative (prospective/retrospective) tiers, with retrospective volume rebates propagated through revenue/AR/commission/ledger as ASC-606 variable consideration; rolling contract-year accrue-vs-settle; rebates per-unit & charger-triggered (product_class qualifying/applies); term/renewal; party.sector. Spec-only; new milestone M-Pricing |
| 25 | **ASSOCIATED_DOCUMENTS** | Inbound / associated documents (attachments) — distinct from doc-17 generated docs, reusing the WORM store: store a received customer PO, signed contract + schedules, delivery note/POD, etc. and associate to an order / party / price_agreement; the PO→order ingestion + provenance + price-reconciliation flow; PII-in-attachment + DSAR interplay. Spec-only (M13-Docs.9) |
| 26 | **REVENUE_FORECASTING** | M-Forecast — self-improving per-account forecasting: rolling-origin censored backtests, the policy tournament + champion-by-argmin selection, empirical P80/P50/P20 bands, seasonality pass-through, and the negative-results register (falsified experiments are documented, never deleted) |
| 27 | **UI_FEATURE_MAP** | The concrete page-by-page design contract for the desk — every feature mapped to its screen, states, and data-layer behavior, with live screenshots in `design-assets/`; §0 is the sign-in/session contract. The design pass's working document (reads before 22) |
| 28 | **PROCUREMENT_ENTITY** | M-Procurement — the principal/LRD structure: SG procurement entity, central transfer-price catalogue per market, flash-title matched journals at dispatch (operating COGS = transfer), full cancellation/return unwind with genealogy, all inter_entity-walled incl. the two-truths entity-structure view; §5 = M-IC-FX, the IC balance lifecycle (ASC 830 remeasurement, settlement runs with realized FX, ASC 815 hedge designation, §482 true-ups — "one moment fixes everything" at dispatch) |
| 29 | **ASSURANCE** | M-Assurance — the proven journal model + gap closure: lifecycle conservation laws (A1, shipped), CTRL-LINEAGE-CLOSURE (A2), the ASC-606 compliance matrix (A3), the route×role authz matrix (B), reproducibility fingerprints (D), coverage/perf/desk-unit/terraform gates (C/E/F/G) |
| 30 | **ENGINEERING_FORMALISM** | The law register — every engineering principle stated as law → mechanism → pinning artifact → origin (the bug that taught it); the per-PR conformance checklist; the amendment procedure. Doc 29 is the backlog strengthening these laws' pinning; this is the register of the laws themselves |
| 31 | **PROOF_CENTER** | M-Proof — the demo book of record (seeded EXCLUSIVELY through production write paths — a SQL-dump demo is an anti-proof) + the interactive formal-proof desk section: the live law register, the Journal Walk (invoice → legs → lineage to the CM PO), the ASC-606 five-step walkthrough (= doc 29 A3's surface), Reconcile, and the non-prod Tamper Sandbox (seeded corruption named live by CTRL-LINEAGE-CLOSURE) |
