...
| M8 Activation + warranty | 🟡 PARTIAL | Live Pulsar path correct. **Warranty lifecycle now real**: all 91,155 activated serials carry a 3yr commercial warranty window (activation + 36mo + extension; statutory 24mo is the floor). **Unit-replacement genealogy + warranty inheritance** wired — `serial_unit.replaces_serial_unit_id` + `rma_ticket` (HubSpot support tickets), boot ignition resolves serials → links the chain → propagates the root warranty_end down (replacement never resets the clock), `GET /serials/{id}/lifecycle` shows the family timeline. Dormant until a real HubSpot ticket export lands (no fabricated links). **Free-shipment classifier** (`free_shipment`): the COGS-without-revenue population is categorised distinctly — warranty_or_rma_replacement (2,655 / £1.21m), sample_converted (481 / £465k), sample_prospect (1,051 / £322k), r_and_d (22, Hypervolt/engineering — expensed to R&D not samples), marketplace_return (Amazon = returns, never samples). Derived from transparent source-backed rules (each row carries its basis), human-overridable, rebuilt 6h + on `/free-shipments/{summary,trend,warranty-metrics,rebuild,reclassify}`. **Warranty/RMA replacement rate = 17.99%** (avg £455) → the accrual basis for forward warranty liabilities. |
