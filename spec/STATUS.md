...
3. **IgnitionService** at boot (idempotent, `IGNITE=false` to disable) — Volex supplier, stamp operating entity onto orders, mint costed opening lots + link serials, open periods, build the exposure forecast, and **emit `dispatch.created` events** → relay → `RevenueRecognitionConsumer` → TigerBeetle (recognition converges async, ~4.5 min). `recognized_at = dispatch date` ⇒ the P&L re-projects to the right fiscal period automatically.
