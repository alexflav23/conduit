...
| /activations desk (ghost-busters port) | 🟢 DONE | Subroutes: Capacity (connected-MW + DC comparison) · Live feed (real activations, auto-refresh 15s) · Analytics (monthly count/MW bars + KPIs). Backend: feed + capacity + series + kpis. Remaining nice-to-have: true SSE server-push (prod streams from the Athena placement topic; polling covers it now). |
