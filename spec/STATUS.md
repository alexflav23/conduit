...
| /activations desk (ghost-busters port) | 🟢 DONE (replica in progress) | Subroutes: Capacity (connected-MW + DC comparison — net-new vs GB) · Live feed (**real-time SSE stream**, ● live + new-glow) · Analytics (monthly count/MW bars + KPIs). Backend: feed + capacity + series + kpis + **SSE /activations/stream**. Remaining for 1:1 replica: richer charts (daily 30/90, yearly + 7-day MA + weekday/weekend baselines, day-of-week, quarterly), frequency KPI, day-navigation, serial→account deep-links. |
...
- **CLEAN LOCK achieved (2025-10 LOCKED)**: issued 19,406 invoices tying AR↔invoices to the penny (£43,969,414.56), TB↔GL matched exactly (£63.25m); inventory↔count + gl↔Xero signed off as documented environment limitations (opening-INV needs in-network TB; no Xero here). Finance closed, admin locked — SoD enforced, lock-gate enforced (0 unsigned exceptions). The full auditable close→lock cycle is demonstrated on real data.
