...
Implemented and verified end-to-end: **Phases 1 & 2 complete (M0–M10 incl. M9b), plus M11 + M12, M13, and M13b complete** (M13b adds the Option-B `gl_entry` ledger read-side + hedge-aware consolidation).
...
> **Last updated through M13 (complete): Xero + dispatch-invoice + credit terms + recognition/P&L + GL +
> documents (multi-locale incl. CJK/Thai) + the real tax/customs engine + engine-owned sales VAT
> (quote → placement → recognition) on a per-jurisdiction immutable ledger.** Update this register as each milestone lands.
...
- ✅ **GL/AR trial balance off the `gl_entry` mirror** (`GlProjectionService`, ties out Σdr==Σcr) — see the M13b gl_entry read-side below; `GET /finance/gl/trial-balance` + `/finance/gl/as-of` are live.
...
- ✅ **`gl_entry` ledger read-side (Option B, doc 14 §5)** — a faithful, immutable Postgres MIRROR of every TigerBeetle posting, written by a single `Journal` at post time (TB byte-identical, then gl_entry, both idempotent). One row per transfer-side tagged with the realisation `phase`, so the posted balance == TB `debits_posted`/`credits_posted` (handles two-phase commission + cross-currency intercompany). All 10 posting services route through `Journal`. So reconciliation / GL / finance reads run as plain SQL with **no TB fan-out on the request path** — and the `gl_vs_tb` MIRROR check proves zero drift (`PeriodCloseSuite` asserts mirror==TB after a real sale).
- ✅ **Reconciliation engine** (`ReconciliationService`, off `gl_entry`) — `ar_vs_invoices` (AR net vs open invoices) + `tb_vs_gl` (whole-ledger Σdr==Σcr) run synchronously in the API; `gl_vs_tb` (the TB mirror tie) runs in the consumer (the one place that reads TB). Expected/actual/variance/status + sign-off; an unsigned exception blocks the lock. **REST**: `POST /finance/periods/{id}/reconciliations/run`, `POST /finance/reconciliations/{rid}/sign-off`, `GET /finance/gl/trial-balance`, `GET /finance/gl/as-of` (layer-gated).
- ✅ **Hedge-aware consolidation** (`ConsolidationService`, ASC 830 — doc 14 §2.4 / doc 13 §7.2) — each entity's native as-of balances translated to the presentation currency at a **provenanced rate, hedge-locked where a hedge is designated** (the contracted rate, not spot), else closing/spot. Recorded as an **immutable `consolidation_run` + lines** (which rate / which hedge per line) so the figure re-derives exactly; CTA is the cross-entity translation residual, FX_CLEARING nets to ~0 — both re-performed as controls (CTRL-CTA-BALANCE, CTRL-FXCLEARING-ZERO). Native facts are never stored translated (a re-projection, like fiscal-period assignment). `ConsolidationSuite`: GBP at closing + EUR at its locked hedge, controls pass, lineage re-derivable. **REST**: `POST /finance/consolidation/run`, `GET /finance/consolidation/{id}`.
- ✅ **Control runner** (`ControlRunner`) — re-performable `evidence_query` → pass/fail + `control_run`; seeded CTRL-DOC-GAPLESS / CTRL-RECON-EXCEPTIONS / CTRL-INV-CONSERVATION + **CTRL-GL-MIRROR / CTRL-HEDGE-DRAWDOWN / CTRL-FXRATE-COMPLETE** (SQL) and the consolidation-computed CTRL-CTA-BALANCE / CTRL-FXCLEARING-ZERO.
- ✅ **Auditability lineage** (`LineageService`) — figure → order_invoice → ledger transfer ids → events → issued PDF; consolidated figure → `consolidation_run` → lines (rate/hedge provenance) → native account.
...
- ✅ **All four reconciliation types + evidence export** — `inventory_vs_count` (INV ledger vs on-hand at specific batch cost) and `gl_vs_xero` (every non-void invoice reached Xero via `xero_invoice_id`; the unsynced value is the variance) join `ar_vs_invoices` + `tb_vs_gl`/`gl_vs_tb`, all gl_entry/Postgres-backed and run together by `POST …/reconciliations/run`. **Evidence pack** (`EvidenceService`, `GET /finance/periods/{id}/evidence`) bundles the period's reconciliations + control runs into a WORM, hash-stamped (`content_sha256`, stamp-independent) bundle for the auditor (`ReconciliationTypesSuite`).
