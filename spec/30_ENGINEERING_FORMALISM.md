...
| **L9** | **Maker–checker, and the checker can see what they approve.** Governed changes (prices, catalogues, stock ops, DSAR shreds) need a second human; `checker ≠ maker` is enforced; the checker's role must hold view rights on the object class it approves — *no role acts on what it cannot view, and none edits a layer it cannot view (edit ⊆ view)*. Scope grants compose as entity ∧ market(geography) ∧ channel ∧ sector ("UK Wholesale, energy"). | Propose/activate pairs everywhere; permission seeds; the four scope axes (`Grant`/`PolicyEngine`/`ScopePredicate`). | `AuthzMatrixSuite` (act ⇒ view; edit ⊆ view; the wall matrix; the sector filter) generated from the seeds + FieldLayerMap; `PolicyEngineSpec`. | V1_0_61: the CEO could not view the tax rates it approved. `AuthzMatrixSuite` (slice B) then found five more act-without-view gaps in the seed and fixed them (V1_0_71/72) — the matrix now keeps the class structurally unreintroducible. |
...
| **L12** | **Reproducibility.** Same data + same code ⇒ bit-identical output, on any machine. Data is pinned by ingest git SHA; deterministic ids (L4) make replay byte-stable. | git-NDJSON ingest snapshots; `Fingerprint` (id-independent count+sum digest per key, folded with the ingest SHA) + `CTRL-REPRO`; the refresher commits `ingest/fingerprint.json` per cycle (doc 29 slice D). | `FingerprintSpec` (order/id-invariance, money-sensitivity, SHA-binding); `ReproSuite` (the demo book digests identically twice despite fresh-UUID churn; a seeded drift fails `CTRL-REPRO` then clears). | The 2026-06-12 cross-machine 16,500 question — now settled: a `(scope, ingest SHA)` that produced two distinct digests is non-determinism or drift, and the control surfaces it. |

## 5. Assurance

| # | Law | Mechanism | Pinned by | Origin |
|---|---|---|---|---|
| **L13** | **Controls are re-performable, and detection is proven.** Every audit-grade invariant is a `CTRL-*` row in the in-product register an auditor can re-run on live data. A control is not done until a test **seeds the corruption it claims to detect** and watches it fail with the precise identity of the break. | The controls register (26 active: CTA-BALANCE, DLQ-EMPTY, DOC-GAPLESS, FXCLEARING-ZERO, FXRATE-COMPLETE, GL-MIRROR, HEDGE-DRAWDOWN, HEDGE-PERF, IC-CATALOGUE, IC-MATCH, IC-REMEASURE, IC-SETTLE-ZERO, IC-TRUEUP, INV-CONSERVATION, LINEAGE-CLOSURE, OUTBOX-DRAINED, PII-SHRED, REBATE-ACCRUAL, RECON-EXCEPTIONS, REPRO, TAX-EXT-EVIDENCE, TAX-NEXUS-GATE, TAX-REPRO, TAX-VAT-CONSERVE, VAT-NO-OVER-REMIT; HEDGE-LOCK superseded by the economic-MTM model, slice 4b). The Journal Atlas (doc 29 A3.2) cross-references every posting site → legs → claim home → reversal story → pin. | Per-control api-it suites; `CTRL-LINEAGE-CLOSURE` (doc 29 A2, V1_0_64) closes lineage bidirectionally — its corollary law, **if you post it, you record it**: every posted leg's deterministic id lands in a fact-table claim column, stamped iff the leg was posted; `LineageClosureSuite` proves detection by seeded corruption (deleted leg, orphan transfer, one-sided mirror, stripped reversal leg — each named precisely). | Doc 14 SOX/ICFR design; the M-Assurance review found controls existed but detection-proof was uneven — and the A2 claims audit found six posting sites with computed-but-ephemeral leg ids (incl. a random event id in stock counts, an L4 breach). |
