...
| S2.1 | HubSpot live API — companies, contacts, deals (company assoc + pipeline), line_items (deal_line), company→company branch hierarchy (BranchLinkService); support tickets = follow-on | S2 | ✅ |
| S2.2 | MRPeasy live API — orders + shipments + serials (stock-lots/POs = follow-on) | S2 | 🟡 |
