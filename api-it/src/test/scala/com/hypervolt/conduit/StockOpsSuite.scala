...
object StockOpsSuite extends IOSuite {
  ...
  private def newVariant(xa: HikariTransactor[IO], serialised: Boolean): IO[(UUID, UUID, UUID)] =
    (for {
      ...
      v   <- sql"INSERT INTO product_variant (family_id, sku, generation, is_serialised) VALUES ($fam, ${s"K-${UUID.randomUUID()}"}, 'v3', $serialised) RETURNING id".query[UUID].unique
  ...
  private def batch(xa: HikariTransactor[IO], v: UUID, costUsd: String): IO[UUID] =
    LotBatchRepo.create(NewBatch(s"B-${UUID.randomUUID()}", None, v, 1, BigDecimal(costUsd), BigDecimal("1.0"), "spot", None, BigDecimal("0"), BigDecimal("0"), "GBP"), LocalDate.parse("2026-01-01")).transact(xa)

  test("write-off is maker-checker: self-approval rejected; a second approver posts the movement + ledger write-down at batch cost") { case (xa, client) =>
    ...
    for {
      ...
      _   <- ledger.createAccounts(List(LedgerAccount(svc.invAccount(e), gbp, LedgerAccountCode.Inv), LedgerAccount(svc.writeOffAccount(e), gbp, LedgerAccountCode.Inv)))
      ...
      moves  <- sql"SELECT count(*) FROM stock_movement WHERE type='write_off' AND ref_id=$adjId".query[Long].unique.transact(xa)
  ...
  test("cycle-count variance is maker-checker and posts a count_correction + ledger movement at batch cost") { case (xa, client) =>
    val svc = new StockOpsService[IO](xa, TigerBeetleLedger.fromClient[IO](client))
    val ledger = TigerBeetleLedger.fromClient[IO](client)
    for {
      ...
      corr    <- sql"SELECT count(*) FROM stock_movement WHERE type='count_correction' AND ref_id=$countId".query[Long].unique.transact(xa)
      invBal  <- ledger.balance(svc.invAccount(e))
    } yield expect(selfNo.isLeft) and expect(ok.isRight) and
      expect(onHand == 8) and expect(corr == 1L) and expect(invBal.creditsPosted == BigInt(10000)) // 2 units * £50
  }

  test("a location transfer goes requested -> in_transit (maker-checker) -> received") { case (xa, client) =>
    val svc = new StockOpsService[IO](xa, TigerBeetleLedger.fromClient[IO](client))
    for {
      ids <- newVariant(xa, serialised = false)
      (e, v, locA) = ids
      ...
      aMid   <- sql"SELECT qty_on_hand FROM stock_item WHERE product_variant_id=$v AND location_id=$locA".query[Int].unique.transact(xa)
      _      <- svc.receiveTransfer(tid)
      bFinal <- sql"SELECT qty_on_hand FROM stock_item WHERE product_variant_id=$v AND location_id=$locB".query[Int].unique.transact(xa)
