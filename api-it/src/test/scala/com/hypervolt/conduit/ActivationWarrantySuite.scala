...
object ActivationWarrantySuite extends IOSuite {
    ...
    (for {
      ...
      fam <-
        sql"INSERT INTO product_family (code, name) VALUES (${s"f-${UUID.randomUUID()}"}, 'Fam') RETURNING id"
          ...
          .unique
      v <-
        sql"INSERT INTO product_variant (family_id, sku, generation) VALUES ($fam, ${s"SKU-${UUID.randomUUID()}"}, $generation) RETURNING id"
          .query[UUID]
          .unique
  ...
  test("first-write-wins, V2 ignored, idempotent redelivery; provision opens at the unit's batch cost") { xa =>
    ...
    for {
      ...
      provisions <-
        sql"SELECT count(*) FROM warranty_provision WHERE serial_unit_id = $sid".query[Long].unique.transact(xa)
      estimated <-
        sql"SELECT estimated_provision FROM warranty_provision WHERE serial_unit_id = $sid"
          .query[BigDecimal]
          .unique
          .transact(xa)
  ...
  test("straight-line release advances exposure; consolidated exposure sums open provisions") { xa =>
    ...
    for {
      ...
      released <-
        sql"SELECT released_to_date FROM warranty_provision WHERE id = $pid".query[BigDecimal].unique.transact(xa)
