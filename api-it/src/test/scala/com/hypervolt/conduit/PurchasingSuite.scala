...
object PurchasingSuite extends IOSuite {
  ...
  test("receiving lands the per-lot cost, increments stock, and auto-fills the oldest backorder") { xa =>
    ...
    for {
      setup <- (for {
          ...
          o <-
            sql"""INSERT INTO "order" (order_no, type, entity_id, sold_to_party_id, bill_to_party_id, status, txn_currency, payment_method)
                              VALUES ('ORD-'||nextval('order_no_seq'),'trade',$e,$p,$p,'placed','GBP','stripe') RETURNING id"""
              .query[UUID]
              .unique
          line <-
            sql"INSERT INTO order_line (order_id, product_variant_id, qty, unit_price_ex_vat, status) VALUES ($o,$v,5,587.50,'backordered') RETURNING id"
              .query[UUID]
              .unique
      ...
      batchId <- purch.receive(
        poId,
        e,
        loc,
        ReceiveLine(plId, v, 10, BigDecimal("100.00"), BigDecimal("1.0"), BigDecimal("0"), BigDecimal("0"), Nil, "GBP"),
