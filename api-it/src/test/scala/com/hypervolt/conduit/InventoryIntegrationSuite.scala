...
object InventoryIntegrationSuite extends IOSuite {
  ...
  private def newEntity: ConnectionIO[UUID] =
    sql"INSERT INTO entity (name, jurisdiction, functional_currency, entity_type) VALUES ('UK Ltd', 'GB', 'GBP', 'operating') RETURNING id".query[UUID].unique
  ...
  private def newVariant(serialised: Boolean): ConnectionIO[UUID] =
    for {
      ...
      v   <- sql"INSERT INTO product_variant (family_id, sku, generation, is_serialised) VALUES ($fam, ${s"SKU-${UUID.randomUUID()}"}, 'v3', $serialised) RETURNING id".query[UUID].unique
    } yield v
  ...
  private def newOrder(entity: UUID): ConnectionIO[UUID] =
    for {
      ...
      o <- sql"""INSERT INTO "order" (order_no, type, entity_id, sold_to_party_id, bill_to_party_id, channel_id, market_id, status, txn_currency, payment_method)
                 VALUES ('ORD-' || nextval('order_no_seq'), 'trade', $entity, $p, $p, ${UUID.randomUUID()}, ${UUID.randomUUID()}, 'placed', 'GBP', 'stripe') RETURNING id""".query[UUID].unique
    } yield o
  ...
  private def newTranche(line: UUID, seq: Int, qty: Int): ConnectionIO[UUID] =
    sql"INSERT INTO delivery_tranche (order_line_id, seq, qty, requested_date) VALUES ($line, $seq, $qty, '2026-07-01') RETURNING id".query[UUID].unique
  ...
  test("a 500-unit line scheduled 2x250 allocates and dispatches tranches independently, invoicing per drop") { xa =>
    for {
      ...
      delivered <- sql"SELECT count(*) FROM outbox_event WHERE event_type='dispatch.delivered' AND aggregate_id=$o".query[Long].unique.transact(xa)
