...
object ReturnsSuite extends IOSuite {
  ...
  private def svcFor(xa: HikariTransactor[IO], client: Client): ReturnService[IO] =
    new ReturnService[IO](xa, TigerBeetleLedger.fromClient[IO](client))
  ...
  test(
    "full_unit return: lifecycle emits return.* events; restock re-recognises INV at batch cost; AR + commission reverse; credit note issued"
  ) {
    case (xa, client) =>
      ...
      for {
        ...
        rma <- svc.raise(
          ...
          "changed_mind",
          maker,
          List(RaiseLine(s.line, Some(s.serial), None, 1))
        )
  ...
  test("maker cannot be checker (self-approval rejected)") {
    case (xa, client) =>
      val svc = svcFor(xa, client)
      for {
        s <- setup(xa)
  ...
  test("serials never silently re-enter sellable stock: restock rejected for non-A-grade and for activated units") {
    case (xa, client) =>
      val svc    = svcFor(xa, client)
      val ledger = TigerBeetleLedger.fromClient[IO](client)
      for {
        ...
        rma <-
          svc.raise(s.order, "full_unit", "serial", "faulty", maker, List(RaiseLine(s.line, Some(s.serial), None, 1)))
        lineId <- sql"SELECT id FROM rma_line WHERE rma_id=$rma".query[UUID].unique.transact(xa)
        ...
        serSt     <- sql"SELECT status FROM serial_unit WHERE serial_no=${s.serial}".query[String].unique.transact(xa)
      } yield expect(bGrade.isLeft) and expect(activated.isLeft) and expect(refurb.isRight) and expect(
        serSt == "refurbished"
      )
  }

  test("scrap disposition books no inventory re-recognition; serial scrapped") {
    case (xa, client) =>
      val svc    = svcFor(xa, client)
      val ledger = TigerBeetleLedger.fromClient[IO](client)
      for {
  ...
  test("part_only return retains commission (no claw)") {
    case (xa, client) =>
      val svc    = svcFor(xa, client)
      val ledger = TigerBeetleLedger.fromClient[IO](client)
      for {
        ...
        rma <- svc.raise(
          ...
          "wrong_item",
          maker,
          List(RaiseLine(s.line, None, Some("cable"), 1))
        )
        lineId <- sql"SELECT id FROM rma_line WHERE rma_id=$rma".query[UUID].unique.transact(xa)
        _      <- svc.assess(rma, List((lineId, "a")), checker)
        _      <- svc.approve(rma, checker, None)
  ...
  test("state-machine invariants: refund before approve / disposition before receive / double refund are rejected") {
    case (xa, client) =>
      val svc    = svcFor(xa, client)
      val ledger = TigerBeetleLedger.fromClient[IO](client)
      for {
