...
object BatchIntegrationSuite extends IOSuite {
    ...
    for {
      fam <-
        sql"INSERT INTO product_family (code, name) VALUES (${s"f-${UUID.randomUUID()}"}, 'Fam') RETURNING id"
          ...
          .unique
      v <-
        sql"INSERT INTO product_variant (family_id, sku, generation) VALUES ($fam, ${s"SKU-${UUID.randomUUID()}"}, 'v3') RETURNING id"
          .query[UUID]
          .unique
    } yield v
  ...
  test("two lots of one SKU carry different landed costs (price, freight, FX differ)") { xa =>
    (for {
      v <- newVariant
      b1 <- LotBatchRepo.create(
        batch("B1", v, "100.00", "0.7900", "spot", "500.00", "200.00"),
        LocalDate.parse("2026-06-01")
      )
      b2 <- LotBatchRepo.create(
        batch("B2", v, "110.00", "0.8100", "hedged", "800.00", "300.00"),
  ...
  test("each serial resolves its own lot's landed cost (specific-identification, no averaging)") { xa =>
    ...
    } yield expect(cost1 != cost2) and expect(cost1 == batch1Cost) and expect(cost1.contains(BigDecimal("79.0000"))))
      .transact(xa)
  }
    ...
    (for {
      ...
      ord <-
        sql"""INSERT INTO "order" (order_no, type, entity_id, sold_to_party_id, bill_to_party_id, status, txn_currency, payment_method)
                     VALUES ('ORD-'||nextval('order_no_seq'),'trade',$e,$party,$party,'placed','GBP','stripe') RETURNING id"""
          .query[UUID]
          .unique
      line <-
        sql"INSERT INTO order_line (order_id, product_variant_id, qty, unit_price_ex_vat) VALUES ($ord,$v,1,587.50) RETURNING id"
          .query[UUID]
          .unique
