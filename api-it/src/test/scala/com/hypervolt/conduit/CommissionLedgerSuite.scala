...
object CommissionLedgerSuite extends IOSuite {
  ...
  test("accrue posts a PENDING transfer; post earns it (posted credits on COMM_PAYABLE)") {
    case (xa, client) =>
      ...
      for {
        ...
        _ <- ledger.createAccounts(
          List(
            LedgerAccount(svc.expenseAccount("GBP"), gbp, LedgerAccountCode.CommPayable),
            LedgerAccount(payable, gbp, LedgerAccountCode.CommPayable)
          )
        )
  ...
  test("commission trues up to the actual batch margin (delta booked as a current-period adjustment)") {
    case (xa, client) =>
      val (svc, ledger) = service(xa, client)
      val gbp           = Ledgers.forCurrency(Currency.GBP)
      for {
  ...
  test("claw voids the pending accrual (no posted credit)") {
    case (xa, client) =>
      val (svc, ledger) = service(xa, client)
      val gbp           = Ledgers.forCurrency(Currency.GBP)
      for {
