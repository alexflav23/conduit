...
object DealDeskHttpSuite extends IOSuite {
  ...
  private def app(xa: HikariTransactor[IO]): HttpApp[IO] = {
    ...
    (new AccessRoutes[IO](xa, auth).routes <+> new CommerceRoutes[IO](xa, auth).routes <+> new DealDeskRoutes[IO](xa, auth).routes).orNotFound
  }

  private def post(xa: HikariTransactor[IO], path: String, token: String, body: Json): IO[Response[IO]] =
    app(xa).run(Request[IO](Method.POST, Uri.unsafeFromString(path)).withEntity(body).putHeaders(Authorization(Credentials.Token(AuthScheme.Bearer, token))))

  private def get(xa: HikariTransactor[IO], path: String, token: String): IO[Response[IO]] =
    app(xa).run(Request[IO](Method.GET, Uri.unsafeFromString(path)).putHeaders(Authorization(Credentials.Token(AuthScheme.Bearer, token))))
  ...
  private def seedCatalogue(xa: HikariTransactor[IO]): IO[Unit] =
    (for {
      fam <- sql"INSERT INTO product_family (code, name) VALUES (${s"f-${UUID.randomUUID()}"},'Home 3 Pro') RETURNING id".query[UUID].unique
      _   <- sql"INSERT INTO product_variant (family_id, sku, generation) VALUES ($fam,'HV-310','v3') ON CONFLICT (sku) DO NOTHING".update.run
      v   <- sql"SELECT id FROM product_variant WHERE sku='HV-310'".query[UUID].unique
      _ <- sql"""INSERT INTO price_rule (surface, product_variant_id, channel_id, market_id, currency, tax_regime, authorised_price, max_discount_pct, min_qty, status)
  ...
  private def party(xa: HikariTransactor[IO]): IO[UUID] =
    sql"INSERT INTO party (display_name, party_type, is_organization) VALUES ('Cust','wholesaler',true) RETURNING id".query[UUID].unique.transact(xa)
  ...
  private def placeOutOfBand(xa: HikariTransactor[IO], agent: String): IO[(UUID, UUID)] =
    for {
      ...
      bill <- party(xa)
      body  = Json.obj(
                "type" -> "trade".asJson, "soldToPartyId" -> sold.toString.asJson, "billToPartyId" -> bill.toString.asJson,
  ...
  test("the Deal Desk workflow: agent proposes a narrative; only the CEO can approve; approval is timed, volume-contingent and customer-specific; the order releases") { xa =>
    for {
      _      <- seedCatalogue(xa)
      ...
      narrative = Json.obj(
                    ...
                    "volumeExpectation" -> 500.asJson, "volumeDenomination" -> "P50".asJson,
      ...
      adminDecision <- post(xa, s"/api/v1/adlp/exceptions/$excId/decision", s"dev:$admin", Json.obj("decision" -> "approve".asJson, "memo" -> "x".asJson))
      // the CEO cannot approve without a memo
      ceoNoMemo <- post(xa, s"/api/v1/adlp/exceptions/$excId/decision", s"dev:$ceo", Json.obj("decision" -> "approve".asJson))
      // the CEO approves: timed (validity window) + volume-contingent (min 400) + memo
      ceoApprove <- post(xa, s"/api/v1/adlp/exceptions/$excId/decision", s"dev:$ceo",
                      Json.obj("decision" -> "approve".asJson, "memo" -> "Approved for Octopus; 500-unit commitment".asJson,
                               "validFrom" -> "2026-06-01T00:00:00Z".asJson, "validTo" -> "2026-09-01T00:00:00Z".asJson, "volumeMin" -> 400.asJson))
      ...
      approvedEv  <- sql"SELECT count(*) FROM outbox_event WHERE event_type='adlp.exception.approved' AND aggregate_id=$orderId".query[Long].unique.transact(xa)
      releasedEv  <- sql"SELECT count(*) FROM outbox_event WHERE event_type='order.placed' AND aggregate_id=$orderId".query[Long].unique.transact(xa)
    } yield {
  ...
  test("a volume-only viewer sees the exception but not its price banding (layer projection)") { xa =>
    for {
      ...
      placed <- placeOutOfBand(xa, agent)
      (_, excId) = placed
      _    <- post(xa, s"/api/v1/adlp/exceptions/$excId/submit", s"dev:$agent",
