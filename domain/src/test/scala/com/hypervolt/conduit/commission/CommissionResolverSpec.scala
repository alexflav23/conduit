...
object CommissionResolverSpec extends SimpleIOSuite {
  ...
  private def scheme(rate: String, exc: String = "zero", from: String = "2026-01-01T00:00:00Z", to: Option[String] = None): CommissionScheme =
    CommissionScheme(UUID.randomUUID(), "gross_margin", BigDecimal(rate), exc, Instant.parse(from), to.map(Instant.parse))
  ...
  pureTest("a more specific assignment beats a general one, within the validity window") {
    ...
    expect(CommissionResolver.resolve(candidates, Some(team), channel, market, None, now).map(_.ratePct).contains(BigDecimal("10")))
  }
  ...
  pureTest("a scheme outside its validity window does not resolve") {
    ...
    expect(CommissionResolver.resolve(List(assign(expired, Some(team), None, None)), Some(team), channel, market, None, now).isEmpty)
  }
  ...
  pureTest("gross-margin commission; zero on an unapproved exception line") {
    ...
    val standard = CommissionResolver.lineCommission(s, CommissionLineInput(BigDecimal("587.50"), BigDecimal("400.00"), 2, "standard", exceptionApproved = false))
    val unapproved = CommissionResolver.lineCommission(s, CommissionLineInput(BigDecimal("587.50"), BigDecimal("400.00"), 2, "exception", exceptionApproved = false))
