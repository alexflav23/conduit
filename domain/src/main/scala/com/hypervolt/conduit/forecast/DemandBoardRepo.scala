...
object DemandBoardRepo {
  ...
  private def rowJson(label: String, key: String, monthly: List[(String, BigDecimal)], shipped: Long, asp: BigDecimal, contributors: Option[List[Json]]): Json = {
    ...
    val revenue = (forecast * asp).setScale(2, BigDecimal.RoundingMode.HALF_UP)
  ...
  def board(market: UUID, scenario: UUID, contributorsPerSegment: Int): ConnectionIO[Json] =
    marketMonthly(market, scenario).flatMap { mkt =>
      val year = mkt.headOption.map(_._1.take(4).toInt).getOrElse(2026)
      (accounts(year), aspBySegment).mapN { (accts, asp) =>
      ...
      val segments = bySegment.toList.map {
        case (seg, members) =>
          ...
          val contribs = members.sortBy(-_.activations).take(contributorsPerSegment).map { a =>
            rowJson(a.name, a.company.toString, monthlyFor(BigDecimal(a.activations) / totalAct), a.shipped, aspOf(seg), None)
          }
          (members.map(_.activations).sum, rowJson(SegLabel.getOrElse(seg, seg), seg, segMonthly, segShipped, aspOf(seg), Some(contribs)))
