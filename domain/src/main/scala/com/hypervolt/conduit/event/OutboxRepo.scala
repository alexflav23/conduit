  ...
  def append(e: OutboxEvent): ConnectionIO[Int] =
    sql"""INSERT INTO outbox_event
            (event_id, event_type, schema_version, aggregate_type, aggregate_id, partition_key,
             scope, correlation_id, causation_id, payload, occurred_at)
          VALUES (${e.eventId}, ${e.eventType}, ${e.schemaVersion}, ${e.aggregateType}, ${e.aggregateId},
             ${e.partitionKey}, ${e.scope}, ${e.correlationId}, ${e.causationId}, ${e.payload}, ${e.occurredAt})
       """.update.run

  private type Row =
    (Long, UUID, String, Int, String, UUID, String, Option[Json], Option[UUID], Option[UUID], Json, Instant)
