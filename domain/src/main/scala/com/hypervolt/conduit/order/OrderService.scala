...
final class OrderService[F[_]: Async](xa: Transactor[F]) {
  ...
  def amend(
      orderId: UUID,
      newLines: List[PlaceLineInput],
      reason: Option[String],
      actor: Option[UUID],
      asOf: Instant
  ): F[Either[OrderError, PlacedOrder]] =
    OrderRepo
      .header(orderId)
      .flatMap {
        ...
        case Some(h) =>
          ...
          if (!amendable)
            OrderError
              .AmendRejected(s"order not amendable (status=${h.status})")
              .asLeft[PlacedOrder]
  ...
  private def totals(priced: List[LinePricing]): (BigDecimal, BigDecimal, BigDecimal) = {
    val subtotal = priced
      .map(p => (p.priced.unitPriceExVat * BigDecimal(p.priced.qty)).setScale(2, RoundingMode.HALF_UP))
  ...
  private def creditCheck(paymentMethod: String, billTo: UUID, total: BigDecimal): ConnectionIO[Option[OrderError]] =
    if (paymentMethod == "stripe") Option.empty[OrderError].pure[ConnectionIO]
      ...
      OrderRepo.creditProfile(billTo).flatMap {
        case None =>
          Option[OrderError](OrderError.NotBillable("no credit profile and not a card payment")).pure[ConnectionIO]
        case Some(cp) =>
          OrderRepo.openOrdersTotal(billTo).map { open =>
            val exposure = open + total
            if (exposure > cp.creditLimit && cp.policy == "block")
              Some(OrderError.CreditBlocked(exposure - cp.creditLimit))
            else None
          }
      }
  ...
  private def auditOrder(orderId: UUID, orderNo: String, status: String, actor: Option[UUID]): ConnectionIO[Int] =
    sql"""INSERT INTO audit_log (entity_type, entity_id, action, after, actor_user_id)
          VALUES ('order', $orderId, 'create', ${Json.obj(
      ...
      priced.map(lp =>
        Json.obj(
          ...
          "tranches" -> Json.fromValues(
            (if (lp.line.schedule.nonEmpty) lp.line.schedule else Nil).map(t =>
              Json.obj("seq" -> t.seq.asJson, "qty" -> t.qty.asJson)
            )
          )
        )
      )
  ...
  private def placedEvent(
    ...
    OutboxEvent(
      ...
      Some(
        Json.obj(
          "entity_id"  -> in.entityId.map(_.toString).asJson,
          "market_id"  -> in.marketId.toString.asJson,
          "channel_id" -> in.channelId.toString.asJson
        )
      ),
      None,
      None,
      Json.obj(
        "order_no"      -> orderNo.asJson,
        "sold_to"       -> in.soldToPartyId.toString.asJson,
        "bill_to"       -> in.billToPartyId.toString.asJson,
