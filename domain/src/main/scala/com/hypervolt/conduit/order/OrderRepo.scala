...
object OrderRepo {
  ...
  def insertException(orderId: UUID, lineId: UUID, requestedPrice: BigDecimal, discountPct: BigDecimal): ConnectionIO[Int] =
  ...
  def snapshotLines(orderId: UUID): ConnectionIO[Json] =
    sql"SELECT product_variant_id, qty, unit_price_ex_vat, adlp_category FROM order_line WHERE order_id = $orderId"
      ...
      .to[List]
      .map(rows =>
        Json.fromValues(rows.map { case (variant, qty, price, adlp) =>
          Json.obj("variant" -> variant.toString.asJson, "qty" -> qty.asJson, "unit_price_ex_vat" -> price.toString.asJson, "adlp" -> adlp.asJson)
        })
      )
  ...
  def insertAmendment(orderId: UUID, actor: Option[UUID], before: io.circe.Json, after: io.circe.Json, reason: Option[String]): ConnectionIO[Int] = {
  ...
  def scopeRow(orderId: UUID): ConnectionIO[Option[(Option[UUID], Option[UUID], Option[UUID], Option[UUID])]] =
    sql"""SELECT entity_id, market_id, channel_id, created_by FROM "order" WHERE id = $orderId"""
      .query[(Option[UUID], Option[UUID], Option[UUID], Option[UUID])].option

  def viewJson(orderId: UUID): ConnectionIO[Option[Json]] =
    for {
      hdr <- sql"""SELECT order_no, status, adlp_category, subtotal_ex_vat, vat_total, total_inc_vat
                   FROM "order" WHERE id = $orderId"""
               .query[(String, String, String, BigDecimal, BigDecimal, BigDecimal)].option
      lines <- sql"""SELECT id, product_variant_id, qty, unit_price_ex_vat, adlp_category, is_scheduled, status
                     FROM order_line WHERE order_id = $orderId"""
                 .query[(UUID, UUID, Int, BigDecimal, String, Boolean, String)].to[List]
      tranches <- sql"""SELECT dt.order_line_id, dt.seq, dt.qty, dt.status FROM delivery_tranche dt
                        JOIN order_line ol ON ol.id = dt.order_line_id WHERE ol.order_id = $orderId"""
                    .query[(UUID, Int, Int, String)].to[List]
    } yield hdr.map { case (no, st, adlp, sub, vat, tot) =>
      ...
      Json.obj(
        ...
        "lines" -> Json.fromValues(lines.map { case (lid, v, q, p, a, sched, lst) =>
          Json.obj(
            ...
            "tranches" -> Json.fromValues(
              byLine.getOrElse(lid, Nil).sortBy(_._2).map { case (_, seq, tq, ts) =>
