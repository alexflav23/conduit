...
final case class ScopeColumns(entity: Fragment, market: Fragment, channel: Fragment, owner: Fragment, sector: Fragment)

object ScopeColumns {
  // `sector` is left NULL by default — routes serving sector-bearing rows (parties, sector-scoped lists)
  // pass a ScopeColumns with the real column; others get an always-true sector clause.
  val default: ScopeColumns =
    ScopeColumns(fr"entity_id", fr"market_id", fr"channel_id", fr"owner_user_id", fr"NULL::text")
}
      ...
      breadth match {
        ...
        case Breadth.Scoped =>
          val parts = List(
            ...
            NonEmptyList.fromList(grant.scopeSectors.toList).map(Fragments.in(cols.sector, _))
          ).flatten
