...
final class DealDeskService[F[_]: Async](xa: Transactor[F]) {
  ...
  def listJson(principal: Principal, status: Option[String]): F[List[Json]] = {
    val sel =
      fr"""SELECT e.id, e.order_id, e.order_line_id, e.party_id, e.list_price, e.max_discount_pct, e.requested_price,
  ...
  private def scopeFilter(principal: Principal): Fragment = {
    val breadths = principal.grants.flatMap(g =>
      g.permissions.collect {
        case p if p.objectType == "adlp_exception" && p.action == Action.View =>
          g.breadthOverride.getOrElse(p.dataBreadth)
      }
    )
  ...
  def submit(exceptionId: UUID, n: Narrative, actor: UUID): F[Either[String, Unit]] =
    orderOfException(exceptionId).transact(xa).flatMap {
      ...
      case Some((orderId, status)) =>
        if (status != "pending_ceo" && status != "draft")
          s"exception not editable in status $status".asLeft[Unit].pure[F]
        else
          (for {
            _ <- captureBanding(exceptionId)
            _ <-
              sql"""UPDATE adlp_exception SET justification = ${n.justification}, volume_expectation = ${n.volumeExpectation},
            ...
            _ <-
              sql"INSERT INTO audit_log (entity_type, entity_id, action, actor_user_id) VALUES ('adlp_exception', $exceptionId, 'submit_narrative', $actor)".update.run
            _ <- OutboxRepo.append(
              event(
                exceptionId,
                orderId,
                "adlp.exception.requested",
                Json.obj(
                  "volume_expectation"  -> n.volumeExpectation.asJson,
                  "volume_denomination" -> n.volumeDenomination.asJson
                )
              )
            )
      ...
      case Some((orderId, status, createdBy)) =>
        if (status != "pending_ceo") s"exception not pending (status $status)".asLeft[Unit].pure[F]
          ...
          (for {
            ...
            _ <-
              sql"UPDATE order_line SET adlp_category='exception', status='open' WHERE id=(SELECT order_line_id FROM adlp_exception WHERE id=$exceptionId)".update.run
            _ <- OutboxRepo.append(
              event(
                exceptionId,
                orderId,
                "adlp.exception.approved",
                Json.obj(
                  "approved_by" -> approver.toString.asJson,
                  "valid_from"  -> validFrom.map(_.toString).asJson,
                  "valid_to"    -> validTo.map(_.toString).asJson,
                  "volume_min"  -> volumeMin.asJson
                )
              )
            )
            ...
            _ <-
              if (released)
                OutboxRepo.append(
                  event(exceptionId, orderId, "order.placed", Json.obj("released_from" -> "pending_ceo".asJson))
                )
          ...
          (sql"UPDATE adlp_exception SET status='rejected', approved_by=$approver, approval_memo_ref=$memo, decided_at=now() WHERE id=$exceptionId".update.run *>
            OutboxRepo.append(
              event(
                exceptionId,
                orderId,
                "adlp.exception.rejected",
                Json.obj("rejected_by" -> approver.toString.asJson)
              )
  ...
  private def releaseOrderIfReady(orderId: UUID): ConnectionIO[Boolean] =
    sql"SELECT count(*) FROM adlp_exception WHERE order_id = $orderId AND status NOT IN ('approved','rejected')"
      .query[Long]
      .unique
      .flatMap { pending =>
        if (pending == 0L)
          sql"""UPDATE "order" SET status='placed', updated_at=now() WHERE id=$orderId AND status='pending_ceo'""".update.run
            .map(_ > 0)
  ...
  private def event(exceptionId: UUID, orderId: UUID, eventType: String, payload: Json): OutboxEvent =
    OutboxEvent(
      ...
      None,
      None,
      payload.deepMerge(Json.obj("exception_id" -> exceptionId.toString.asJson)),
      Instant.now()
    )
}
