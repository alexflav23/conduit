...
final class ReturnService[F[_]: Async](xa: Transactor[F], ledger: TigerBeetleLedger[F]) {
  ...
  def raise(orderId: UUID, rType: String, scope: String, reasonCode: String, requestedBy: UUID, lines: List[RaiseLine]): F[UUID] =
    (for {
      ord <- sql"""SELECT entity_id, sold_to_party_id, bill_to_party_id, channel_id, market_id, txn_currency
                   FROM "order" WHERE id = $orderId""".query[(Option[UUID], UUID, UUID, Option[UUID], Option[UUID], String)].unique
      (entity, soldTo, billTo, channel, market, currency) = ord
      rmaId <- sql"""INSERT INTO rma (rma_no, order_id, entity_id, sold_to_party_id, bill_to_party_id, channel_id, market_id,
                     ...
                     VALUES ('RMA-' || nextval('rma_no_seq'), $orderId, $entity, $soldTo, $billTo, $channel, $market,
                       $rType, $scope, $reasonCode, $currency, 'raised', $requestedBy) RETURNING id""".query[UUID].unique
      _ <- lines.traverse_(l => insertRmaLine(rmaId, l))
      _ <- OutboxRepo.append(event(rmaId, "return.raised", Json.obj("order_id" -> orderId.toString.asJson, "type" -> rType.asJson, "reason_code" -> reasonCode.asJson)))
  ...
  private def insertRmaLine(rmaId: UUID, l: RaiseLine): ConnectionIO[Unit] =
    for {
      ...
      commEntry <- sql"SELECT id FROM commission_entry WHERE order_id = (SELECT order_id FROM order_line WHERE id = ${l.orderLineId}) AND status='posted' LIMIT 1".query[UUID].option
  ...
  private def serialMeta(serialNo: String): ConnectionIO[Option[(Option[UUID], Option[UUID], Option[BigDecimal])]] =
    sql"""SELECT s.id, s.lot_batch_id, b.landed_unit_cost FROM serial_unit s
      ...
      .query[(UUID, Option[UUID], Option[BigDecimal])].option.map(_.map { case (sid, b, c) => (Some(sid), b, c) })

  // ----- assess -----

  def assess(rmaId: UUID, grades: List[(UUID, String)], assessedBy: UUID): F[Unit] =
    (grades.traverse_ { case (lineId, grade) => sql"UPDATE rma_line SET condition_grade = $grade, status='assessed' WHERE id = $lineId".update.run.void } *>
      sql"UPDATE rma SET status='assessed', assessed_by=$assessedBy, updated_at=now() WHERE id=$rmaId".update.run *>
      OutboxRepo.append(event(rmaId, "return.assessed", Json.obj("assessed_by" -> assessedBy.toString.asJson)))).transact(xa).void
          ...
          ruleFor(h.rType).transact(xa).flatMap { rule =>
            if (rule.requiresMemo && memo.isEmpty) "approval memo required for this return type".asLeft[Unit].pure[F]
              ...
              (for {
                ...
                _ <- sql"UPDATE rma SET status='approved', approved_by=$approver, refund_amount=$refund, approval_memo_ref=$memo, updated_at=now() WHERE id=$rmaId".update.run
                replacementId <- if (rule.issuesReplacement) issueReplacement(rmaId, h).map(Some(_)) else Option.empty[UUID].pure[ConnectionIO]
                _ <- replacementId.fold(().pure[ConnectionIO])(rid =>
                       sql"UPDATE rma SET replacement_order_id=$rid WHERE id=$rmaId".update.run.void *>
                         OutboxRepo.append(event(rmaId, "return.replaced", Json.obj("replacement_order_id" -> rid.toString.asJson, "priced" -> rule.replacementPriced.asJson))).void)
                _ <- if (h.rType == "warranty_replacement") drawDownWarranty(rmaId) else ().pure[ConnectionIO]
                _ <- OutboxRepo.append(event(rmaId, "return.approved", Json.obj("approved_by" -> approver.toString.asJson, "refund_amount" -> refund.toString.asJson, "commission_claw_armed" -> (rule.commissionTreatment == "claw").asJson)))
  ...
  def disposition(rmaId: UUID, lineId: UUID, choice: String, locationId: Option[UUID], actor: UUID): F[Either[String, Unit]] =
    statusOf(rmaId).transact(xa).flatMap {
  ...
  private def dispositionLine(rmaId: UUID, lineId: UUID, choice: String, locationId: Option[UUID], actor: UUID): F[Either[String, Unit]] =
          ...
          case Right(_) =>
            ...
            val invLeg = choice match {
              case "restock" | "refurbish" if line.entityId.isDefined && cost > 0 =>
                ledger.postTransfers(List(LedgerTransfer(TbIds.transferId(lineId, 1), invAccount(line.entityId.get), cosClearing(line.entityId.get), minor(cost), gbpLedger, LedgerTransferCode.Generic)))
              case _ => Async[F].unit
            }
  ...
  private def postDisposition(rmaId: UUID, line: RmaLineRow, choice: String, locationId: Option[UUID], actor: UUID): ConnectionIO[Unit] = {
    ...
    for {
      mv <- sql"""INSERT INTO stock_movement (type, product_variant_id, location_id, entity_id, qty, ref_type, ref_id, reason_code, actor_user_id)
                  VALUES ($movementType, ${line.variant}, ${locationId.orElse(line.locationId)}, ${line.entityId}, $qty, 'rma', $rmaId, 'return', $actor) RETURNING id""".query[UUID].unique
      ...
      _ <- sql"""INSERT INTO return_disposition (rma_line_id, serial_unit_id, disposition, from_status, to_status, location_id, stock_movement_id, actor_user_id)
      ...
      _ <- sql"UPDATE rma_line SET status='dispositioned', disposition=$choice, restock_location_id=$locationId WHERE id=${line.id}".update.run
      _ <- OutboxRepo.append(event(rmaId, "return.restocked", Json.obj("rma_line_id" -> line.id.toString.asJson, "disposition" -> choice.asJson, "to_status" -> newStatus.asJson)))
    } yield ()
  }
        ...
        if (h.status == "raised" || h.status == "assessed") "cannot refund before approval".asLeft[Unit].pure[F]
          ...
          ruleFor(h.rType).transact(xa).flatMap { rule =>
            ...
            for {
              ...
              _ <- if (refund > 0) ledger.postTransfers(List(
                     LedgerTransfer(TbIds.transferId(rmaId, 10), revenueAccount(entity), arAccount(h.billTo), minor(refund), gbpLedger, LedgerTransferCode.Generic),
                     LedgerTransfer(TbIds.transferId(rmaId, 11), vatAccount(entity), arAccount(h.billTo), minor(vat), gbpLedger, LedgerTransferCode.Generic)
                   )) else Async[F].unit
              _ <- if (refund > 0) issueCreditNote(rmaId, h, refund, vat, method).transact(xa) else ().pure[F]
              _ <- if (rule.commissionTreatment == "claw") clawCommission(rmaId).transact(xa).flatMap(effectClaw) else ().pure[F]
              _ <- sql"UPDATE rma SET status='refunded', tb_reversal_group=${TbIds.transferId(rmaId, 10).toString}, updated_at=now() WHERE id=$rmaId".update.run.transact(xa)
              _ <- OutboxRepo.append(event(rmaId, "return.refunded", Json.obj("refund_method" -> method.asJson, "total_inc_vat" -> (refund + vat).toString.asJson))).transact(xa)
            } yield ().asRight[String]
          }
    }

  private def issueCreditNote(rmaId: UUID, h: RmaHeader, ex: BigDecimal, vat: BigDecimal, method: String): ConnectionIO[Unit] =
    for {
      cnId <- sql"""INSERT INTO credit_note (rma_id, order_id, credit_note_no, bill_to_party_id, total_ex_vat, vat_total, total_inc_vat, refund_method)
                    VALUES ($rmaId, ${h.orderId}, 'CN-' || nextval('credit_note_no_seq'), ${h.billTo}, $ex, $vat, ${ex + vat}, $method) RETURNING id""".query[UUID].unique
  ...
  private def effectClaw(claw: Option[(UUID, UUID, BigDecimal)]): F[Unit] =
    claw.fold(Async[F].unit) { case (entryId, agentId, amount) =>
      ledger.postTransfers(List(LedgerTransfer(TbIds.transferId(entryId, 9), commPayable(agentId), commExpense(agentId), minor(amount), gbpLedger, LedgerTransferCode.Commission))) *>
        sql"""INSERT INTO commission_entry (agent_id, scheme_id, order_id, basis_amount, rate_applied, amount, currency, kind, status, tb_transfer_id)
              SELECT agent_id, scheme_id, order_id, -basis_amount, rate_applied, -amount, currency, 'claw', 'clawed', ${TbIds.transferId(entryId, 9).toString}
  ...
  private def loadHeader(rmaId: UUID): ConnectionIO[Option[RmaHeader]] =
    sql"""SELECT order_id, entity_id, sold_to_party_id, bill_to_party_id, channel_id, market_id, type, status,
      ...
      .query[(UUID, Option[UUID], UUID, UUID, Option[UUID], Option[UUID], String, String, String, Option[BigDecimal], UUID)]
      .option
...
private final case class ReturnRule(refundBasis: String, issuesReplacement: Boolean, replacementPriced: Boolean, defaultDisposition: String, commissionTreatment: String, warrantyEffect: String, requiresMemo: Boolean, approvalThreshold: Option[BigDecimal])
private final case class RmaHeader(orderId: UUID, entityId: Option[UUID], soldTo: UUID, billTo: UUID, channel: Option[UUID], market: Option[UUID], rType: String, status: String, currency: String, refundAmount: Option[BigDecimal], requestedBy: UUID)
private final case class RmaLineRow(id: UUID, variant: UUID, serialId: Option[UUID], grade: Option[String], unitLandedCost: Option[BigDecimal], entityId: Option[UUID], locationId: Option[UUID], activated: Boolean)
