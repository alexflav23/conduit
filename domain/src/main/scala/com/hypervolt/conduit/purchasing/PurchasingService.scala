...
final class PurchasingService[F[_]: Async](xa: Transactor[F], alloc: AllocationService[F]) {
  ...
  def createPO(entity: UUID, supplier: Option[UUID], currency: String): F[(UUID, String)] =
    sql"""INSERT INTO purchase_order (po_no, entity_id, supplier_id, txn_currency)
      ...
      .query[(UUID, String)].unique.transact(xa)
    ...
    val tx: ConnectionIO[UUID] =
      for {
        batchId <- LotBatchRepo.create(
                     NewBatch(batchNo, None, line.variantId, line.qty, line.unitCostUsd, line.fxRate, "spot", None, line.freight, line.duty, line.currency),
                     receivedDate
                   )
        ...
        _ <- sql"INSERT INTO goods_receipt_line (grn_id, po_line_id, qty_received, serials, lot_batch_id) VALUES ($grn, ${line.poLineId}, ${line.qty}, ${line.serials}, $batchId)".update.run
  ...
  private def recordLanded(grn: UUID, poId: UUID, kind: String, amount: BigDecimal, currency: String): ConnectionIO[Int] =
    if (amount.signum <= 0) Applicative[ConnectionIO].pure(0)
    else sql"INSERT INTO landed_cost_component (grn_id, po_id, type, amount, currency) VALUES ($grn, $poId, $kind, $amount, $currency)".update.run
  ...
  def autoAllocateBackorders(entity: UUID, variant: UUID): F[Unit] =
    sql"""SELECT ol.id, (ol.qty - ol.qty_allocated)
      ...
      _.traverse_ { case (lineId, needed) => alloc.allocate(lineId, None, entity, variant, needed, serialised = false).void }
    }
}
