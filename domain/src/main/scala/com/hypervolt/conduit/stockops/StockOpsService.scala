...
final class StockOpsService[F[_]: Async](xa: Transactor[F], ledger: TigerBeetleLedger[F]) {
  ...
  private def postAdjustment(
    ...
    for {
      ...
      _ <-
        sql"UPDATE stock_item SET qty_on_hand = qty_on_hand - $qty, updated_at = now() WHERE entity_id = $entity AND product_variant_id = $variant AND location_id = $location".update.run
      _ <- serials.traverse_(s => sql"UPDATE serial_unit SET status = 'scrapped' WHERE serial_no = $s".update.run.void)
      _ <- sql"UPDATE stock_adjustment SET status = 'posted', approved_by = $approver, tb_transfer_id = ${TbIds
        .transferId(adjustmentId, 0)
        .toString} WHERE id = $adjustmentId".update.run
    } yield ()
  ...
  def submitCount(entity: UUID, location: UUID, lines: List[(UUID, Int, Int)], countedBy: UUID): F[UUID] =
    (for {
      countId <-
        sql"INSERT INTO stock_count (entity_id, location_id, status, counted_by) VALUES ($entity, $location, 'pending_approval', $countedBy) RETURNING id"
          .query[UUID]
          .unique
      _ <- lines.traverse_ {
        case (variant, counted, system) =>
  ...
  def approveCount(countId: UUID, approver: UUID): F[Either[String, Unit]] =
    sql"SELECT entity_id, location_id, counted_by, status FROM stock_count WHERE id = $countId"
  ...
  def requestTransfer(from: UUID, to: UUID, entity: UUID, variant: UUID, qty: Int, requestedBy: UUID): F[UUID] =
    sql"""INSERT INTO stock_transfer (from_location_id, to_location_id, entity_id, product_variant_id, qty, status, requested_by)
          VALUES ($from, $to, $entity, $variant, $qty, 'requested', $requestedBy) RETURNING id"""
      .query[UUID]
      .unique
      .transact(xa)
  ...
  def approveTransfer(transferId: UUID, approver: UUID): F[Either[String, Unit]] =
    sql"SELECT from_location_id, entity_id, product_variant_id, qty, requested_by, status FROM stock_transfer WHERE id = $transferId"
      .query[(UUID, Option[UUID], UUID, Int, UUID, String)]
      .option
      .transact(xa)
      .flatMap {
  ...
  def receiveTransfer(transferId: UUID): F[Unit] =
    sql"SELECT to_location_id, entity_id, product_variant_id, qty FROM stock_transfer WHERE id = $transferId AND status = 'in_transit'"
      .query[(UUID, Option[UUID], UUID, Int)]
      .option
      .flatMap {
  ...
  private def serialCost(serialNo: String): ConnectionIO[Option[BigDecimal]] =
    sql"SELECT b.landed_unit_cost FROM serial_unit s JOIN lot_batch b ON b.id = s.lot_batch_id WHERE s.serial_no = $serialNo"
      .query[BigDecimal]
      .option

  private def variantCost(variant: UUID): F[BigDecimal] =
    sql"SELECT landed_unit_cost FROM lot_batch WHERE product_variant_id = $variant ORDER BY received_date DESC LIMIT 1"
      .query[BigDecimal]
      .option
      .transact(xa)
