...
private final case class ValidatedLine(lineId: UUID, variant: UUID, qty: Int, serials: List[String], serialised: Boolean)
...
final class DispatchService[F[_]: Async](xa: Transactor[F]) {
      ...
      .flatMap {
        ...
        case Right(validated) =>
          for {
            ...
            dispatchId <- sql"""INSERT INTO dispatch (dispatch_no, order_id, tranche_id, carrier_id, tracking_no, status)
                                VALUES ($dispatchNo, $orderId, $trancheId, $carrierId, $tracking, 'created') RETURNING id""".query[UUID].unique
            _ <- validated.traverse_(v => shipLine(dispatchId, trancheId, v))
            _ <- sql"""UPDATE "order" SET dispatched_at = COALESCE(dispatched_at, now()), updated_at = now() WHERE id = $orderId""".update.run
            _ <- OutboxRepo.append(event(orderId, "dispatch.created", Json.obj("dispatch_no" -> dispatchNo.asJson, "tranche_id" -> trancheId.map(_.toString).asJson)))
          } yield dispatchId.asRight[String]
      }
      .transact(xa)

  def deliver(dispatchId: UUID): F[Either[String, String]] =
    sql"SELECT order_id, tranche_id FROM dispatch WHERE id = $dispatchId".query[(UUID, Option[UUID])].option.flatMap {
      ...
      case Some((orderId, tranche)) =>
        for {
          ...
          _         <- tranche.fold(Sync0)(t => sql"UPDATE delivery_tranche SET status = 'delivered' WHERE id = $t".update.run.void)
          invoiceNo <- sql"SELECT 'INV-' || nextval('invoice_no_seq')".query[String].unique
          _ <- sql"""INSERT INTO order_invoice (order_id, tranche_id, invoice_no, total_ex_vat, vat_total, total_inc_vat)
                     SELECT $orderId, $tranche, $invoiceNo, subtotal_ex_vat, vat_total, total_inc_vat FROM "order" WHERE id = $orderId""".update.run
          _ <- tranche.fold(Sync0)(t => sql"UPDATE delivery_tranche SET status = 'invoiced' WHERE id = $t".update.run.void)
          _ <- OutboxRepo.append(event(orderId, "dispatch.delivered", Json.obj("dispatch_id" -> dispatchId.toString.asJson, "tranche_id" -> tranche.map(_.toString).asJson)))
          _ <- OutboxRepo.append(event(orderId, "order.invoiced", Json.obj("invoice_no" -> invoiceNo.asJson, "tranche_id" -> tranche.map(_.toString).asJson)))
  ...
  private def validateLine(in: DispatchLineInput): ConnectionIO[Either[String, ValidatedLine]] =
    sql"""SELECT pv.is_serialised, ol.product_variant_id FROM order_line ol
      ...
      .query[(Boolean, UUID)].option.map {
        case None => Left(s"unknown order line ${in.orderLineId}")
        case Some((serialised, variant)) =>
          if (serialised && in.serials.size != in.qty) Left(s"serialised line requires ${in.qty} serials, got ${in.serials.size}")
  ...
  private def shipLine(dispatchId: UUID, tranche: Option[UUID], v: ValidatedLine): ConnectionIO[Unit] =
    for {
      _ <- sql"INSERT INTO dispatch_line (dispatch_id, order_line_id, tranche_id, qty) VALUES ($dispatchId, ${v.lineId}, $tranche, ${v.qty})".update.run
      _ <- if (v.serialised) v.serials.traverse_(s => shipSerial(dispatchId, s)) else shipNonSerial(v.lineId, tranche, v.qty)
      _ <- sql"UPDATE order_line SET qty_dispatched = qty_dispatched + ${v.qty}, status = 'dispatched' WHERE id = ${v.lineId}".update.run
      _ <- tranche.fold(Sync0)(t => sql"UPDATE delivery_tranche SET qty_dispatched = qty_dispatched + ${v.qty}, status = 'dispatched', dispatch_id = $dispatchId WHERE id = $t".update.run.void)
    } yield ()
  ...
  private def shipSerial(dispatchId: UUID, serialNo: String): ConnectionIO[Unit] =
    sql"SELECT id, product_variant_id, location_id, entity_id FROM serial_unit WHERE serial_no = $serialNo"
      .query[(UUID, UUID, Option[UUID], Option[UUID])].unique.flatMap { case (sid, variant, locOpt, entOpt) =>
  ...
  private def event(orderId: UUID, eventType: String, payload: Json): OutboxEvent =
    OutboxEvent(UUID.randomUUID(), eventType, 1, "order", orderId, orderId.toString, None, None, None, payload, Instant.now())
}
