...
final case class CreatePartyReq(displayName: String, partyType: String, isOrganization: Boolean, channelId: Option[String], marketId: Option[String], customerPoRequired: Option[Boolean])
object CreatePartyReq { implicit val codec: Codec[CreatePartyReq] = deriveCodec }

final case class BillingProfileReq(billingName: String, currency: String, paymentTermsDays: Int, taxRegimeDefault: Option[String])
...
final class CommerceRoutes[F[_]: Async](xa: Transactor[F], auth: AuthService[F]) {
  ...
  private def orderErrorStatus(e: OrderError): (StatusCode, ApiError) = e match {
  ...
  private def toLineInputs(lines: List[OrderLineReq]): Either[(StatusCode, ApiError), List[PlaceLineInput]] =
    lines.traverse { l =>
      for {
        ...
        schedule  <- l.schedule.getOrElse(Nil).traverse(t => date(t.requestedDate).map(d => TrancheInput(t.seq, t.qty, d)))
  ...
  private val createParty =
    base.post.in("api" / "v1" / "parties").in(jsonBody[CreatePartyReq]).out(statusCode(StatusCode.Created).and(jsonBody[Json]))
      .serverLogic(_ => req =>
        (for { ch <- optUuid(req.channelId); mk <- optUuid(req.marketId) } yield (ch, mk)) match {
          case Left(e) => Async[F].pure(Left(e))
          case Right((ch, mk)) =>
            PartyRepo.create(req.displayName, req.partyType, req.isOrganization, ch, mk, req.customerPoRequired.getOrElse(false))
              .transact(xa).map(id => Right(Json.obj("id" -> id.toString.asJson)))
        }
      )

  private val billingProfile =
    base.post.in("api" / "v1" / "parties" / path[String]("id") / "billing-profile").in(jsonBody[BillingProfileReq]).out(statusCode(StatusCode.Created).and(jsonBody[Json]))
      .serverLogic(_ => { case (idStr, req) =>
        uuid(idStr) match {
          case Left(e) => Async[F].pure(Left(e))
          case Right(pid) =>
            PartyRepo.addBillingProfile(pid, req.billingName, req.currency, req.paymentTermsDays, req.taxRegimeDefault)
              .transact(xa).map(id => Right(Json.obj("id" -> id.toString.asJson)))
        }
      })

  private val creditProfile =
    base.post.in("api" / "v1" / "parties" / path[String]("id") / "credit-profile").in(jsonBody[CreditProfileReq]).out(statusCode(StatusCode.Created).and(jsonBody[Json]))
      .serverLogic(_ => { case (idStr, req) =>
        (for { pid <- uuid(idStr); limit <- decimal(req.creditLimit) } yield (pid, limit)) match {
          case Left(e) => Async[F].pure(Left(e))
          case Right((pid, limit)) =>
            PartyRepo.addCreditProfile(pid, limit, req.currency, req.termsDays, req.policy, "self")
              .transact(xa).map(id => Right(Json.obj("id" -> id.toString.asJson)))
        }
      })
  ...
  private val placeOrder =
    base.post.in("api" / "v1" / "orders").in(jsonBody[CreateOrderReq]).out(statusCode.and(jsonBody[Json]))
      .serverLogic(principal => req => {
        val parsed = for {
          ...
          _ <- Either.cond(
                 PolicyEngine.authorize(principal, Action.Create, "order", Target(entity, Some(market), Some(channel), Some(principal.userId))),
                 (), err(StatusCode.Forbidden, "forbidden", "requires create:order")
               )
        } yield PlaceOrderInput(req.`type`, entity, soldTo, billTo, channel, market, req.currency, req.paymentMethod, req.customerPoNumber, reqDel, Some(principal.userId), lines)
        parsed match {
          case Left(e) => Async[F].pure(Left(e))
          case Right(in) =>
  ...
  private val amendOrder =
    base.post.in("api" / "v1" / "orders" / path[String]("id") / "amend").in(jsonBody[AmendReq]).out(statusCode.and(jsonBody[Json]))
  ...
  private val getOrder =
    base.get.in("api" / "v1" / "orders" / path[String]("id")).out(jsonBody[Json])
  ...
  val routes: HttpRoutes[F] =
    Http4sServerInterpreter[F]().toRoutes(List(createParty, billingProfile, creditProfile, placeOrder, amendOrder, getOrder))
}
