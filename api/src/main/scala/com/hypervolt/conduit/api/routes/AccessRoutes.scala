...
object WhoAmI {
  ...
  def from(p: Principal): WhoAmI =
    WhoAmI(p.userId.toString, p.grants.flatMap(_.permissions).map(pm => s"${pm.action.name}:${pm.objectType}").distinct.sorted)
}
...
final class AccessRoutes[F[_]: Async](xa: Transactor[F], auth: AuthService[F]) {
      ...
      .serverLogic(principal =>
        _ =>
          requireAdmin(principal) match {
            ...
            case Right(_) => AdminRepo.listRoles.transact(xa).map(rs => Right(rs.map { case (id, n, p) => RoleDto(id.toString, n, p) }))
          }
      )
    ...
    base.post
      ...
      .serverLogic(principal =>
        req =>
          requireAdmin(principal) match {
            case Left(e)  => Async[F].pure(Left(e))
            case Right(_) => AdminRepo.createRole(req.name, req.description).transact(xa).map(id => Right(RoleDto(id.toString, req.name, isPreset = false)))
          }
      )
  ...
  private val assign =
    base.post
      ...
      .serverLogic(principal => { case (keycloakId, req) =>
        val result = for {
          _        <- requireAdmin(principal)
          roleId   <- Try(UUID.fromString(req.roleId)).toEither.leftMap(_ => (StatusCode.BadRequest, ApiError("bad_request", "invalid roleId")))
